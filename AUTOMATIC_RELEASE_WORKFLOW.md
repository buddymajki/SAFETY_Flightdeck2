# 🚀 FULLY AUTOMATIC RELEASE WORKFLOW

## ✨ Mostantól TELJESEN AUTOMATIKUS!

Ezután **CSAK 4 lépés kell:**

```
1. pubspec.yaml verzió frissítés
   ↓
2. dart bin/update_version.dart
   ↓
3. flutter build apk --release
   ↓
4. git push + git tag
   ↓
   ✅ AUTOMATIKUS: Release + APK + metadata.json uploaded!
```

---

## 📝 DETAILED WORKFLOW

### Step 1: Update Version
```yaml
# pubspec.yaml
version: 1.0.5+2  # Change this
```

### Step 2: Auto-Sync Everything
```bash
dart bin/update_version.dart
```

**Ez automatikusan:**
✅ Frissíti `lib/services/app_version_service.dart`  
✅ Frissíti `metadata.json` (verzió + GitHub download URL)

### Step 3: Build APK
```bash
flutter build apk --release
```

### Step 4: Push to GitHub
```bash
git add .
git commit -m "new Release v1.0.7"
git push origin master
```

### Step 5: Create Release Tag (Triggers GitHub Actions!)
```bash
git tag v1.0.7
git push origin --tags
```

### Step 6: GitHub Actions (AUTOMATIKUS!)

GitHub Actions automatikusan:
✅ Létrehozza a release-t (`v1.0.5`)  
✅ Buildel Flutterben  
✅ Feltölti az APK-t a release-be

---

## ⚙️ HOW IT WORKS

### A Script (update_version.dart)
```dart
pubspec.yaml: version: 1.0.5+2
        ↓ (reads)
Parses: 1.0.5 + 2
        ↓ (generates)
AppVersionService: const String _appVersion = '1.0.5';
metadata.json: {
  "version": "1.0.5",
  "downloadUrl": "https://github.com/.../v1.0.5/app-release.apk",
  ...
}
```

### GitHub Actions (release.yml)
```
git push → GitHub detects (master branch)
        ↓
Builds Flutter APK
        ↓
GitHub Actions creates Release
        ↓
Uploads APK to release
        ↓
✅ Release is live!
```

### What the app does
```
App starts
        ↓
Downloads metadata.json from GitHub (raw)
        ↓
Parses version + downloadUrl
        ↓
If new version: Show update dialog
        ↓
User clicks "Update"
        ↓
Downloads from GitHub Release (direct link)
        ↓
✅ APK installs!
```

---

## 🎯 COMPLETE CHECKLIST FOR EACH RELEASE

### Phase 1: Development (Local)
- [ ] Make code changes
- [ ] Test locally
- [ ] Update `pubspec.yaml` version: `1.0.5+2`

### Phase 2: Prepare Release
```bash
# Terminal
dart bin/update_version.dart          # Auto-sync everything
flutter build apk --release           # Build APK
```
- [ ] `metadata.json` is updated ✅ (auto)
- [ ] `app_version_service.dart` is updated ✅ (auto)
- [ ] APK is built ✅

### Phase 3: Push to GitHub
```bash
git add .
git commit -m "Release v1.0.5"
git push origin master
```
- [ ] Files pushed to GitHub

### Phase 4: Create Release Tag (Triggers GitHub Actions)
```bash
git tag v1.0.5
git push origin --tags
```
- [ ] Tag created and pushed
- [ ] GitHub Actions workflow triggered
### Phase 5: GitHub Actions (AUTOMATIC!)
- [ ] Check: GitHub → Actions → Release workflow running
- [ ] Wait ~5-10 minutes for build
- [ ] Check: GitHub → Releases → New release with APK

### Phase 6: Users Get Update!
- [ ] Users open app
- [ ] Metadata.json downloaded from GitHub
- [ ] Update dialog appears
- [ ] Users install new version

---

## 📊 FILES OVERVIEW

| File | What | Auto-Updated? |
|------|------|---|
| `pubspec.yaml` | Version source | ❌ Manual |
| `metadata.json` | Release info | ✅ Yes |
| `app_version_service.dart` | App version const | ✅ Yes |
| `lib/screens/main_navigation.dart` | Metadata URL | ✅ Constant |
| GitHub Release | APK hosting | ✅ GitHub Actions |

---

## 🔍 IMPORTANT NOTES

### metadata.json is NOW on GitHub!
- Location: `metadata.json` in repo root
- URL: `https://raw.githubusercontent.com/buddymajki/SAFETY_Flightdeck2/master/metadata.json`
- This URL **NEVER CHANGES** ✅
- Is auto-generated by script ✅

### GitHub Release URL Structure
```
https://github.com/buddymajki/SAFETY_Flightdeck2/releases/download/vX.Y.Z/app-release.apk
```
- versionből jön  
- Erről töltödik le az APK  
- GitHub automatikusan generálja

### No More Manual Steps!
❌ GONE: Manual Google Drive uploads  
❌ GONE: Manual metadata.json editing  
❌ GONE: Firestore updates  

✅ NEW: One command syncs everything  
✅ AUTOMATIC: GitHub Actions releases  
✅ AUTOMATIC: Users get updates  

---

## ✨ THE MAGIC

**Before:**
```
pubspec → Manual build → Manual upload Drive → Manual metadata edit → Firestore update
```
❌ 15 manual steps, error-prone

**Now:**
```
pubspec → dart bin/update_version.dart → flutter build apk --release → git push → git tag → GitHub Actions
```
✅ 4 commands, EVERYTHING else automatic!

---

## 🚀 QUICK START NEXT RELEASE

```bash
# 1. Edit pubspec.yaml (version: 1.0.5+2)

# 2. Sync + Build
dart bin/update_version.dart
flutter build apk --release

# 3. Push to GitHub
git add .
git commit -m "Release v1.0.5"
git push origin master

# 4. Create Release Tag (Triggers GitHub Actions!)
git tag v1.0.5
git push origin --tags

# 🤖 Watch GitHub Actions do its magic! 🎉
# → GitHub → Actions → Releases tab
```

---

## 🆘 TROUBLESHOOTING

### Q: GitHub Actions not triggering?
- Make sure you pushed to `master` branch
- Check: GitHub → Actions → Workflows file exists
- Check: `.github/workflows/release.yml` is in repo

### Q: APK not uploading to release?
- Check: GitHub → Actions → Build log
- Common: Flutter build failed → Check `flutter build apk --release` locally
- Fix: Run `flutter pub get` before building

### Q: metadata.json wrong format?
- Script auto-generates it
- Check: `dart bin/update_version.dart` output
- Should see: ✅ metadata.json updated successfully!

### Q: App not seeing new version?
- Check: metadata.json in repo
- Check: Version in JSON > current app version
- Wait: App checks on startup
- Restart: Force close + reopen app

---

## 📚 RELATED FILES

- **GitHub Actions:** `.github/workflows/release.yml`
- **Version Sync Script:** `bin/update_version.dart`
- **Metadata Template:** `metadata.json`
- **App Integration:** `lib/screens/main_navigation.dart`
- **Version Service:** `lib/services/app_version_service.dart`

---

## 🎉 YOU'RE ALL SET!

Everything is now **100% automatic** except:
- Editing `pubspec.yaml` (manual)
- Running `flutter build apk --release` (manual)
- `git push` (manual)

Everything else? 🤖 **AUTOMATIC!**

**Legközelebb: Edit pubspec → Build → Push → Kész!** 🚀
